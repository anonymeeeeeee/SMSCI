...
from smsci.data.loader import *
from smsci.models import TrajectoryGenerator
...
def evaluate(args, loader, generator, num_samples, train_loader_visu):
    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            for (i,donnees) in list(enumerate(train_loader_visu))[j:j+1]:
            ...
            if obs_traj.size(1) >= 32:
              for _ in range(num_samples):
                ...
                ade.append(displacement_error(pred_traj_fake, pred_traj_gt[:,:32,:], mode='raw'))
                fde.append(final_displacement_error(pred_traj_fake[-1], pred_traj_gt[:,:32,:][-1], mode='raw'))
