...
from smsci.models import TrajectoryGenerator, TrajectoryDiscriminator
from smsci.utils import int_tuple, bool_flag, get_total_norm
...
# Dataset options
parser.add_argument('--dataset_name', default='eth', type=str) #zara2, hotel, zara1, university
...
parser.add_argument('--checkpoint_name', default='checkpoint_zar1') #univ
...
def main(args):
    ...
    train_loader_visu = data_loader_visu(64)
    #visual_input_tensor = None
    ...
    while t < args.num_iterations:
        ...
        for batch in train_loader:    
            ...
            if t > 0 and t % args.checkpoint_every == 0:
                ...
                metrics_val = check_accuracy(args, val_loader, generator, discriminator, d_loss_fn, train_loader_visu)
                ...
                # Save a checkpoint with no model weights by making a shallow copy of the checkpoint excluding some items
...
def discriminator_step(args, batch, generator, discriminator, d_loss_fn, optimizer_d):
    ...
    for (i,donnees) in list(enumerate(train_loader_visu)):
        _,pred,visual_obs,_ = donnees
    ...
    obs_traj = obs_traj[:,:32,:]
    pred_traj_gt = pred_traj_gt[:,:32,:]
    obs_traj_rel = obs_traj_rel[:,:32,:]
    pred_traj_gt_rel = pred_traj_gt_rel[:,:32,:]
...
def check_accuracy(args, loader, generator, discriminator, d_loss_fn, train_loader_visu, limit=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            traj_fake = torch.cat([obs_traj[:,:32,:], pred_traj_fake], dim=0)
            traj_fake_rel = torch.cat([obs_traj_rel[:,:32,:], pred_traj_fake_rel], dim=0)
